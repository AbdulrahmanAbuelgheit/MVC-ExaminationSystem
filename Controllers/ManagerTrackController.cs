...
namespace ExaminationSystemMVC.Controllers
{
    ...
    public class ManagerTrackController : Controller
    {
            ...
            try
            {
                ...
                var students = _unit.StudentRepo.GetByTrackAndBranch(id, _managerBranchId)
                    .Select(s => new DisplayStudentVM
                    {
                        ...
                        TrackID = trackId,
                        IsActive = "Active" // Set appropriate status
                    }).ToList();
        ...
        {
            // Verify track belongs to manager's branch
            ...
            // Verify student belongs to manager's branch
        ...
        {
            // Get instructors from the manager's branch only
        ...
        public IActionResult Create(CreateTrackVM vm)
        {
            ...
            {
                // Repopulate instructors if validation fails
                var branchInstructors = _unit.BranchRepo.GetById(_managerBranchId)?.Ins ?? new List<Instructor>();
        ...
        [HttpGet]
        public IActionResult AddInstructor(int id) // TrackID
        {
            // Get instructors in the manager's branch
            var instructors = _unit.InstructorRepo.GetInstructorsByBranchId(_managerBranchId);

            // Get track to check already assigned instructors
        ...
        public IActionResult AssignInstructorToCourse(int courseId, int trackId)
        {
            ...
            // Get instructors in branch not already assigned to course
        ...
        public IActionResult AddInstructor(AddInstructorToTrackVM vm)
        {
            ...
            if (ModelState.IsValid)
            {
                // Verify instructor exists and belongs to manager's branch
        ...
        public IActionResult DeleteTrack(int id)
        {
            ...
            try
            {
                // Remove track from manager's branch only
                var branch = _unit.BranchRepo.GetById(_managerBranchId);
                if (branch != null)
                {
                    // First remove the track from the branch
                    branch.Tracks.Remove(track);
                    _unit.BranchRepo.Update(branch);

                    // Then remove the branch from the track's branch collection
        ...
        public IActionResult DeleteStudent(int id)
        {
            ...
            var trackId = student.TrackID; // Handle null case if TrackID is nullable
            _unit.StudentRepo.Delete(id); // Pass the ID instead of the student object
            _unit.Save();
